...
//       .then(() => getMessages().then(renderMessages));
//       // console.log('Delete clicked', target.getAttribute('data-id'));
//   }
// });
...
function postMessage ({username, content}) {
  const fData = new FormData();
  fData.set('username', username, 'content', content);
...
function renderMessages (messages) {
  ...
  const messagesHTML = messages.map(message => {
    return `
      <li>
        ...
        <p>
          <strong>${message.id}</strong>
          <input type="button" id="flag_${message.id}" name="flag" value="${message.flagged ? 'no flag' : 'flag this'}">
          <a data-id="${message.id}" href class="delete-button">Delete</a>
        </p>
      </li>
    `;
  ...
  messagesUl.innerHTML = messagesHTML;
  // debugger
  const flagButtons = document.querySelectorAll('#messages input')

  flagButtons.forEach((btn) => {
    btn.addEventListener('click', event => {
      const { currentTarget } = event;
      event.preventDefault();
      let btnId = currentTarget.attributes.id.value;
      // console.log(btnId);
      let urlId = parseInt(btnId.substring(5));
      // console.log(urlId);
      fetch(`/messages/${urlId}`, {
